#!/usr/bin/env python3
"""
Optimiert Port 0x3da Handler mit vollständiger Retrace-Zyklus-Vorspulung
"""
...
for i, line in enumerate(lines):
    if 'case 0x3ba:' in line and i+1 < len(lines) and 'case 0x3da:' in lines[i+1]:
        ...
        new_handler = [
            f'{indent}case 0x3ba:\n',
            f'{indent}case 0x3da:\n',
            f'{indent}    /* Fast-forward through entire retrace cycle */\n',
            f'{indent}    uint32_t now = get_uticks();\n',
            f'{indent}    \n',
            f'{indent}    if (s->st01 & ST01_V_RETRACE) {{\n',
            f'{indent}        /* Guest is waiting for VRETRACE to clear (phase 2→0) */\n',
            f'{indent}        s->st01 &= ~(ST01_V_RETRACE | ST01_DISP_ENABLE);\n',
            f'{indent}        s->retrace_phase = 0;\n',
            f'{indent}        s->retrace_time = now + RETRACE_INTERVAL_US;\n',
            f'{indent}    }} else {{\n',
            f'{indent}        /* Guest is waiting for VRETRACE to set (phase 0→1→2) */\n',
            f'{indent}        s->st01 |= ST01_DISP_ENABLE | ST01_V_RETRACE;\n',
...
print("✓ Port 0x3da Optimierung v2 abgeschlossen")
